...
In the `map` function, the variable `file` was chosen to represent each element that is passed to the function. The function emits a tuple where the first element is the organ name, returned by the `getSimpleName()` method, and the second element is the `.fa` file path. 
...
transcriptome_ch = Channel.fromPath("$params.transcriptome_file")
    .map { file -> 
    def group_key = file.getSimpleName()
    [group_key, fasta]
    }
```
:::
...
The `by` option can be used to combine items that share a matching key. This value is zero-based, and represents the index or list of indices for the grouping key. The emitted tuple will consist of multiple elements, where the first element is the grouping key, and the following elements are the values of the input channels. 

For example: `source` and `target` are channels consisting of multiple tuples, where the first element of each tuple represents the grouping key. Since indexing is zero-based, `by` is set to `0` to represent the first element of the tuple. Each value within the `source` and `target` channels are separate elements in the emitted tuple. 
...
Create a channel `quant_inputs_ch` that contains the `reads_ch` combined with the `index_ch` using a matching key. The emitted channel should contain three elements, where the first element is the grouping key, the second element is the path to the salmon index file, and the third element is a list of the `.fq` pairs.
...
The `reads_ch` is combined with the `index_ch` using the `combine` channel operator, and is assigned to the channel `quant_inputs_ch`. The new `quant_inputs_ch` channel is input into the `QT` process. 
...
The `groupTuple` operator collects tuples into groups based on a grouping key, emitting a new tuple for each distinct key. The `groupTuple` differs from the `combine` operator in that it is performed on one input channel, and the matching values are emitted as a list. 
...
In the RNA-seq pipeline, the `fromFilePairs` channel factory has been used to to convert paired `.fq` to a channel of tuples: 
...
```default
>>> mv /scratch/users/.../nf-training/data/ggal/lung_2.fq .
>>> ls /scratch/users/.../nf-training/data/ggal
gut_1.fq
gut_2.fq
gut.transcriptome.fa
...
Use the `fromPath` channel operator to read all `.fq` files. Then, use `map` to create a mapping function that returns a tuple, where the first element is the grouping key, and the second element is the `.fq` file. Then, use `groupTuple()` to  create channels containing both paired and single-end `.fq` files. 

Expected output:
```default
[lung, [/scratch/users/.../nf-training/data/ggal/lung_1.fq]]
[gut, [/scratch/users/.../nf-training/data/ggal/gut_2.fq, /scratch/users/.../nf-training/data/ggal/gut_1.fq]]
[liver, [/scratch/users/.../nf-training/data/ggal/liver_1.fq, /scratch/users/.../nf-training/data/ggal/liver_2.fq]]
```

The grouping key can be defined as the following. `file` is the implicit variable defined inside the function that represents each `.fq` file. The `getName()` method will return the file name without the full path. `replaceAll` is used to remove the `_2.fq` and `_1.fq` file suffixes. 

```default
def group_key = file.getName().replaceAll(/_2.fq/,'').replaceAll(/_1.fq/,'')
```

For `file = /scratch/users/.../nf-training/data/ggal/gut_2.fq`, the defined `group_key` will be `gut`. 
...
The `quant_inputs_ch` should now consist of:
```default
[liver, /scratch/users/.../work/cf/42458b80e050a466d62baf99d0c1cf/salmon_idx, [/scratch/users/.../nf-training/data/ggal/liver_1.fq, /scratch/users/.../nf-training/data/ggal/liver_2.fq]]
...
```
Now that the input `reads` can be either single or paired-end, the `QUANTIFICATION` process needs to be modified to allow for either input type. This can be done using the `flatten()` operator, and conditional script statements. Additionally, the `size()` method can be used to calculate the size of a list. 
...
Currently the `TRIMGALORE` process only accounts for paired reads. Modify the process such that both single and paired-end reads can be used. For single reads, the following script block can be used:
