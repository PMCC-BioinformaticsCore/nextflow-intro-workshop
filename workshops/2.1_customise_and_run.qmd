...
HBR_Rep1_ERCC,fastq/HBR_Rep1_ERCC-Mix2_Build37-ErccTranscripts-chr22.read1.fastq.gz,fastq/HBR_Rep1_ERCC-Mix2_Build37-ErccTranscripts-chr22.read2.fastq.gz,auto
HBR_Rep2_ERCC,fastq/HBR_Rep2_ERCC-Mix2_Build37-ErccTranscripts-chr22.read1.fastq.gz,fastq/HBR_Rep2_ERCC-Mix2_Build37-ErccTranscripts-chr22.read2.fastq.gz,auto
```
...
### **2.1.5. Default nf-core configuration**
...
### **2.1.6. When to use a custom config file**
...
The rest of lesson 2.1 will explore custom **resource** configuration files, while [lesson2.4](2.4_multiConfig.html) will focus on **customising outputs**. We won't be covering custom workflow execution on HPCs in this workshop, but please check out our [tips and tricks](../tips_tricks.html#run-nextflow-on-hpc) page later if you are interested in this!
...
### :skull_and_crossbones: <span style="color:#8b0000;">No nf-core params in custom conmedia!</span>
 When customising nf-core workflows, <span style="color:#8b0000;">**do not add parameters to custom config files!**</span> The case of `max` resource settings is a <span style="color:#8b0000;">**rare exception**</span> to this rule.
...
```default
params {
    config_profile_description  = 'test_resources'
    max_cpus   = 2
    max_memory = 6.GB      
}
```

### **2.1.7. Custom resource configuration using process labels**
...
Save the file then re-run the workflow with our custom configuration, setting `outdir` parameter to `Lesson-2.1.7`:
...
Recall from [lesson 1.2.5](1.2_intro_nf_core.html#default-configuration-files) the following tips: 
...
You can [view the modules.conf file on Github](https://github.com/nf-core/rnaseq/blob/3.11.1/conf/modules.config) or search your local copy:
...
Save the config then resume your run, setting `outdir` to `Lesson-2.1.7`, once again applying your custom resources from `my_resources.config`:
...
- nf-core executes by default with `workflow/nextflow.config` and `workflow/conf/base.config` and has a repository of community-contributed institutional conmedia that ship with the workflow 
- we can write (and contribute) our own institutional config for reproducible runs on our compute platform 
- custom conmedia can be applied to a run with `-c <config_name>`, and will over-ride settings in the default conmedia
