...
```default
nextflow run nf-core/testpipeline -profile test,singularity --outdir Lesson1_2
```
...
```default
nextflow run nf-core/testpipeline -profile test,singularity --multiqc_title koala --outdir Lesson1_2
```
:::
...
Give the MultiQC report for the `nf-core/testpipeline` workflow the name of your favorite color using the [`multiqc_title`](https://github.com/nf-core/testpipeline/blob/master/nextflow.config#L27) parameter in a custom `.config` file:
...
```default
nextflow run nf-core/testpipeline -resume -c my_custom_config.config
```
...
For example, if you were trying to add arguments in the `MULTIQC` process in the `nf-core/testpipeline` workflow, you could use the process scope:
...
In the example above, the nf-core [`MULTIQC`](https://github.com/nf-core/testpipeline/blob/master/modules/nf-core/multiqc/main.nf) module, was called by the [`DEMO`](https://github.com/nf-core/testpipeline/blob/master/workflows/demo.nf) workflow, which was called by the [`NFCORE_DEMO`](https://github.com/nf-core/testpipeline/blob/master/main.nf) workflow in the `main.nf` file.
...
```default
nextflow run nf-core/testpipeline -c my_custom_config.config -resume
```
...
Demonstrate the configuration hierarchy using the `nf-core/testpipeline` workflow by adding a params file (`-params-file`), and a command line flag (`--multiqc_title`) to your execution. You can use the files you have already created.
...
```default
nextflow run nf-core/testpipeline -resume -params-file my_custom_params.json --multiqc_title "koala"
```
...
### **1.2.7. Default configuration files**
